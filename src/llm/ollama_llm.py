    ...
    async def generate(self, messages: list[LLMMessage], **kwargs: Any) -> LLMResponse:
        ...
        try:
            ...
            response = await self._client.chat(
                model=model, messages=formatted_messages, stream=False, **kwargs
            )
            ...
            return LLMResponse(content=content, model=response.get("model", model), usage=usage)
    ...
    async def stream(self, messages: list[LLMMessage], **kwargs: Any) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            stream_response = await self._client.chat(
                model=model, messages=formatted_messages, stream=True, **kwargs
            )
